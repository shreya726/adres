...
def lexical_main(targets, responses, manual_scores = []):
	automated_scores = []
	last3 = []
	errors = 0
	ae = 0
	for i in range(0,len(targets)):
		...
		try:
			...
			(score,response) = scoring(target,response,last3,targets)
			automated_scores += [score]
			#Noting last 3 targets and responses to monitor perseveration.
			if len(last3) <6 and score!=0:
				last3 += [response]
				last3 += [target]
			else:
				if score!=0: 
			...
			if str(score) != manual_scores[i]: 
				errors +=1
				print("ERROR: target = "+target+" response = " + response)
				print("Manual score: " + manual_scores[i])
				print("Automated score: " + str(score))
	return errors

def scoring(target, response, last3,targets):
